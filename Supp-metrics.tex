...
Assume there are S distinguishable expression levels of a gene and there are $N_1,...,N_S$ number of cell type groups in each level. Then, Gini-Simpson index (GSI) is:
\begin{align}
GSI &= 1 - \sum_{i}^{S}{p_{i}^2} \\
    &= 1 - \frac{\sum_{i}^{S}{N_i(N_i-1)}}{N(N-1)} \\
\end{align}
Where $N=\sum_{i}^{S}{N_i}$ is the total number of groups and $p_{i}$ is the probability of randomly selected element being in expression level $i$, so $p_{i}^2=N_i(N_i-1)/{N(N-1)$, for sampling without replacement. 
